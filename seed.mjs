...
// seed.mjs — load the built-in SYSTEM apps (components/*.html) into a registry.
// CLI: node seed.mjs   (idempotent per content: re-seeding same html just bumps version)
// Library: import { seedSystemApps } from "open-mcp-apps" — embedders (e.g. a hosted
...
// Library scene categories for the seeds (design-system §7.5). System apps carry no scene;
// library entries get their taxonomy at install time if ever needed.
const SCENES = {};

// Only SYSTEM apps are installed on seed. The other components/*.html are the LIBRARY —
...
/**
 * Seed the system apps into `store`. Returns [{name, action: "seeded"|"unchanged"|"skipped"|"conflict"|"error", version?, error?}].
...
export function seedSystemApps(store, { log = () => {} } = {}) {
  ...
  for (const file of readdirSync(join(HERE, "components")).filter((f) => f.endsWith(".html"))) {
    ...
    // ever passes scene:null for scene-less system apps (settings/dashboard), where the store's
    ...
    // ONE shipped app must degrade to "that declaration cleared, loudly" — never to an engine
    // that fails to boot its own UI.
    const r = store.execute({ type: "save_app", command_id, name, html, description: DESCRIPTIONS[name] || "", scene, actor: "seed", declaration_policy: "salvage" });

...
import { CONTROL_PLANE_TOOLS } from "./tool-policy.mjs";
...
// callTool — any tier, allowlisted, wildcarded, whatever. Policy and component
// save/delete/restore/install belong to the first-party shell alone; a rendered/previewed
// child must never rewrite policy or overwrite/delete/revert components. Any future library_*
// tool is covered by the prefix rule at the callTool chokepoint. The list is INTERPOLATED from
// tool-policy.mjs (the single source of truth) — settings.html mirrors it via oma
// .isControlPlaneTool, so no copy is hand-maintained and the set can never drift.
const CONTROL_PLANE_DENY = ${JSON.stringify(CONTROL_PLANE_TOOLS)};
  ...
  async function handleChildCall(method, a) {
    switch (method) {
      ...
      case "callTool": {
        ...
        if ((tn === "component_html" || tn === "get_component" || tn === "get_component_version") && caps.read_source !== true) throw new Error("component source read denied");
        // file tools: gate by the file_read / file_write caps, and BIND component to the running
        // one so a child reaches its OWN files only — never another component's (there is no
        // cross-component file cap; own-files-only is the strict default).
        if (tn === "file_read" || tn === "file_list" || tn === "file_usage") {
          if (caps.file_read !== true) throw new Error("file read denied by policy");
          ta.component = name;
        }
        if (tn === "file_write" || tn === "file_write_begin" || tn === "file_write_chunk" || tn === "file_write_commit" || tn === "file_write_abort" || tn === "file_delete") {
          if (caps.file_write !== true) throw new Error("file write denied by policy");
          if (tn === "file_write" || tn === "file_write_begin" || tn === "file_delete") ta.component = name;   // upload-id tools (chunk/commit/abort) inherit begin's binding
        }
...
const TOKEN_VALUE_RE = /^[-a-zA-Z0-9 _.,()%#/'"]+$/; // no <, >, ;, {, }, backslash, newline

...
// tools/components.mjs — the component registry surface: the per-component wiring, the universal opener,
...
import { RO, WRITE, WRITE_NOT_IDEMPOTENT, snapshotSchema, capsShape, cmdArgs, SEEDED_COMPONENTS, RESERVED_COMPONENT_NAMES, LOCKED_COMPONENTS, SCENE_CATEGORIES, tierOf, RUNNER_REQUIRED_HTML, defaultCollectionFor, answer, toMcp, textWindow } from "../contracts.mjs";

// "Does this document actually talk to the API?" — the shapes a real component reaches it by.
...
export function register(ctx) {
  ...
  // to 0. So every component the AI creates re-bills the entire conversation from scratch. The cost
  ...
  function registerComponent(name) {
    ...
    registerAppTool(
      ...
      `open_${name.replaceAll("-", "_")}`,
      {
        title: `Open ${name}`,
        annotations: RO,
        description: `Open the "${name}" component as an interactive widget — use when the user wants to SEE or OPERATE this data (to merely read facts, use data_list instead). Optionally pass a collection name to bind it to a specific data collection (default: the one the component declares, else "${name}").`,
        inputSchema: { collection: z.string().optional().describe(`data collection to bind (default: the component's declared one, else "${name}")`) },
        outputSchema: { ...snapshotSchema, component: z.string().optional(), returned: z.number().optional(), eot: z.string().optional() },
        _meta: { ui: { resourceUri: uri } },
      },
      async (a) => {
        // Zero rows, same ruling as open_component: the widget refetches on mount.
  ...
  registerAppTool(
    ...
    "open_component",
    {
      ...
      inputSchema: {
        ...
        collection: z.string().optional().describe("data collection to bind (default: the one the component declares, else its own name)"),
      },
      outputSchema: { ...snapshotSchema, component: z.string(), returned: z.number().optional(), eot: z.string().optional() },
    ...
    async (a) => {
      ...
      // Binding order: what the caller ASKED for, then what the component DECLARED, then its name.
      ...
      return toMcp(answer.page(
        { component: a.component, collection, items: [], version: v.seq,
          settings_version: v.settings_version, files_version: v.files_version, host: hostName() },
        { returned: 0, total,
          text: `Opened "${a.component}" on collection "${collection}" (${total} item(s), seq ${v.seq}). The widget loads its own data; if YOU need rows, read data_list.` },
      ));
    },
  );
  ...
  server.registerTool(
    "component_html",
    {
      ...
      inputSchema: {
        name: z.string(),
        offset: z.number().optional().describe("window the html (same grammar as get_component); omit for the whole document"),
      ...
      outputSchema: {
        ...
        // two chances to disagree about what a component declared. One source, read once, at save.
    ...
    async (a) => {
      ...
      // get_component, which windows by default.
      ...
      return toMcp(answer.chunk(
        ...
        { returned: w.text.length, total: w.total,
          text: `(component "${comp.name}" v${comp.version} — html chars ${w.offset}–${w.offset + w.text.length} of ${w.total}${w.next_offset != null ? `, continue at offset ${w.next_offset}` : ", end"})` },
      ));
    },
  );
  ...
  server.registerTool(
    "list_components",
    {
      ...
      annotations: RO,
      description: "List UI components in the registry (reusable across all chats). If the UI the user wants already exists, prefer opening it over creating a new one. Lists the user's openable apps by default — pass name to look one up, or widen with kind/visibility.",
      ...
      inputSchema: {
        name: z.string().optional().describe("exact component name — the fastest way to answer \"open my X\""),
    ...
    async (a = {}) => {
      ...
      // them veto a lookup makes the tool report that an existing component does not exist.
      ...
      const own = comps.filter((c) => !SEEDED_COMPONENTS.has(c.name));
      // E10: the "→ open_<name>" pointer is only true when per-component tools are registered,
      ...
      // open_component, which always works, is what it should reach for instead.
      const line = (c) => `- ${c.name} v${c.version} (${c.html_size} chars, by ${c.author})` +
        (SEEDED_COMPONENTS.has(c.name) ? " [ships with the engine — not one of the user's apps]" : "") +
      ...
      const empty = a.name
        ? `No component named "${a.name}". Call list_components with no arguments to see what exists.`
        : scoped
        ? "Nothing matches that filter. Call list_components with no arguments (or kind:\"any\", visibility:\"any\") to widen."
      ...
      const text = comps.length
        ? [head, comps.map(line).join("\n"), own.length ? null :
            ...
            "an answer to what they want. Build one: get_component_guide, then save_component, then open_component."]
            .filter(Boolean).join("\n")
        : empty;
      return { content: [{ type: "text", text }], structuredContent: { total: all.length, shown: comps.length, components: comps } };
    },
  );
  ...
  server.registerTool(
    "get_component",
    {
      ...
      annotations: RO,
      description: "Read a component's HTML source as a WINDOW — offset/length select it, next_offset continues, total is the full length. Windows exist because some hosts silently drop the MIDDLE of an oversized result: a big component read whole can arrive mutilated with no sign, and an edit saved from it destroys the source. Carries version — the expected_version for edit_component / save_component.",
      inputSchema: {
        name: z.string(),
  ...
  server.registerTool(
    "save_component",
    {
      ...
      inputSchema: {
        ...
        expected_version: z.number().optional().describe("REQUIRED when overwriting an existing component: the version you read (get_component). Creating a new name needs none"),
        ...
        manifest: z.any().optional().describe("RETIRED — declare inside the html (see get_component_guide)"),
        scene: z.any().optional().describe("RETIRED — declare inside the html (see get_component_guide)"),
      },
    ...
    async (a) => {
      ...
      // component's lost-reply retry was answered "expected_version_required"). The store still
      ...
      // that never saw the current source is exactly how a stub eats a live component — and on a
      ...
      const existing = store.getComponent(a.name);
      if (existing && a.expected_version == null)
        return toMcp(answer.fail("expected_version_required", { name: a.name, version: existing.version },
          `"${a.name}" already exists at v${existing.version} (${existing.html.length.toLocaleString()} chars). Overwriting requires expected_version — read it first (get_component) and pass expected_version: ${existing.version}. A NEW component needs a different name.`), { isError: true });
      const warnings = [];
      const notes = [];
      ...
      // and a component that guesses a category wrong is still a component worth saving.
      ...
      // Accept every shape a real component reaches the API by, not just the literal `oma.`:
      ...
      if (!r.ok) {
        if (r.conflict) {
          const why = r.deleted
            ...
            : `Version conflict: "${a.name}" is at v${r.expected}, not v${a.expected_version}. Someone saved it since you read it — get_component again and re-apply your change.`;
      ...
      const lines = [
        `Saved "${a.name}" v${r.version}${r.created ? " (new component)" : " (updated)"} — ${sizeNote}`,
        ...notes,
  ...
  server.registerTool(
    "edit_component",
    {
      ...
      annotations: WRITE,
      description: "Surgical edits to a component WITHOUT round-tripping the whole source: each edit replaces old_string (which must match exactly once, or set replace_all) with new_string. All edits apply together against expected_version, or nothing applies. No pre-read needed for source you just saved — your copy is byte-exact and your receipt carries the version; get_component is for source you did not write. The #oma-manifest block is re-read on save.",
      inputSchema: {
        ...cmdArgs,
        // `component`, not `name` — save_component/get_component take `name`, and the split is
        ...
        component: z.string().describe("component name (this tool says `component`; save_component and get_component say `name`)"),
        expected_version: z.number().describe("REQUIRED — the version the edits were authored against (from get_component)"),
    ...
    async (a) => {
      if (RESERVED_COMPONENT_NAMES.has(a.component)) return fail(`"${a.component}" is a reserved namespace.`);
      if (LOCKED_COMPONENTS.has(a.component)) return fail(`"${a.component}" is a locked system component — its UI ships with the engine.`);
      ...
      if (a.command_id) {
        const prior = store.priorReceipt(a.command_id);
        if (prior) {
          if (prior.event_type !== "component_saved" || prior.aggregate_id !== a.component)
            return fail(failNote({ error: "command_id_reused" }));
          return toMcp(answer.ack(
            { ok: true, name: a.component, version: prior.seq, applied: Array.isArray(a.edits) ? a.edits.length : 0,
              note: "already applied (idempotent replay)" },
            `Already applied — "${a.component}" is at v${prior.seq} from this same command_id.`));
        }
      }
      const comp = store.getComponent(a.component);
      if (!comp) return fail(`No component "${a.component}". list_components shows what exists.`);
      if (comp.version !== a.expected_version)
        return toMcp(answer.fail("version_conflict", { name: a.component, expected_version: comp.version },
          `"${a.component}" is at v${comp.version}, not v${a.expected_version} — re-read the region you are editing and re-apply.`), { isError: true });
      ...
      for (let i = 0; i < edits.length; i++) {
        ...
        if (n === 0) return fail(`Edit ${i}: old_string not found (0 matches) — read that region again (get_component with offset) and match it exactly, including whitespace. NOTHING was applied.`);
      ...
      const r = store.execute({
        type: "save_component", command_id: a.command_id, name: a.component, html,
        description: "", actor: a.actor || "agent", host: hostName(), expected_version: a.expected_version,
      });
      if (!r.ok) {
        if (r.conflict) return toMcp(answer.fail("version_conflict", { name: a.component, expected_version: r.expected },
          `Version conflict: "${a.component}" moved to v${r.expected} while editing — re-read and re-apply. NOTHING was applied.`), { isError: true });
      ...
      if (r.idempotent)
        return toMcp(answer.ack({ ok: true, name: a.component, version: r.version, applied: edits.length, note: "already applied (idempotent replay)" },
          `Already applied — "${a.component}" is at v${r.version} from this same command_id.`));
      return toMcp(answer.ack(
        { ok: true, name: a.component, version: r.version, size: r.size, prev_size: r.prev_size ?? null,
          applied: edits.length, declaration: r.declaration, ...(r.note ? { note: r.note } : {}) },
        `Edited "${a.component}" — ${edits.length} edit(s) applied, v${a.expected_version} → v${r.version}, ${(r.prev_size ?? 0).toLocaleString()} → ${r.size.toLocaleString()} chars.` +
          (r.note ? `\n${r.note}` : ""),
      ));
    },
  );
  ...
  server.registerTool(
    ...
    async (a) => {
      if (LOCKED_COMPONENTS.has(a.component)) return fail(`"${a.component}" is a system component — it stays.`);
      const r = run({ ...a, name: a.component }, "archive_component");
      if (!r.ok) return fail(failNote(r));
      const vis = r.visibility ?? store.getComponent(a.component)?.visibility;
      const note = r.unchanged ? `already ${vis}` : r.idempotent ? "already applied (idempotent replay)" : undefined;
      return toMcp(answer.ack(
        { ok: true, name: r.name ?? a.component, visibility: vis, version: r.version, ...(note ? { note } : {}) },
        note ? `"${a.component}" — ${note} (v${r.version}).` : `"${a.component}" is now ${vis} (v${r.version}).`,
      ));
    },
  );
  ...
  // a component that did not exist when this module ran.
